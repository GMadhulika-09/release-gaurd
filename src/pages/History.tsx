import { useState, useEffect } from "react";
  ...
  import { Trash2, RefreshCw, Clock, ShieldCheck, GitCompare, Archive } from "lucide-react";
  ...
  const History = () => {
    ...
    const handleDeleteComparisonEntry = (id: string) => {
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Button variant="outline" size="sm" onClick={handleClearAnalysisHistory}>
                ...
                <CardContent className="space-y-4">
                  ...
                  {history.map((item: any) => (
                    <div key={item.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow" onClick={() => handleSelectEntry(item)}>
                      <div className="flex items-start space-y-2 mb-3">
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="outline" size="sm" onClick={handleClearComparisonHistory}>
                      <Trash2 className="h-4 w-4 mr-2" />
                      Clear All
                    </Button>
                  </div>
                    ...
                    {comparisonHistory.map((entry: any) => (
                      <div key={entry.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                        ...
                        <div className="grid gap-2 sm:grid-cols-3 text-center">
                          <div className="p-2 bg-emerald-500/10 rounded-lg">
                            <p className="text-xs text-muted-foreground uppercase tracking-wider">Added</p>
                            <p className="text-lg font-bold text-emerald-600">{entry.filesAdded || 0}</p>
                          </div>
                          <div className="p-2 bg-amber-500/10 rounded-lg">
                            <p className="text-xs text-muted-foreground uppercase tracking-wider">Modified</p>
                            <p className="text-lg font-bold text-amber-600">{entry.filesModified || 0}</p>
                          </div>
                          <div className="p-2 bg-red-500/10 rounded-lg">
                            <p className="text-xs text-muted-foreground uppercase tracking-wider">Deleted</p>
                            <p className="text-lg font-bold text-red-600">{entry.filesDeleted || 0}</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 p-3 bg-muted/30 rounded-lg">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-sm">
                            <div>
                              <p className="text-xs text-muted-foreground">Previous</p>
                              <p className="font-mono font-bold">{entry.previousRisk || 0}</p>
                              <p className="text-xs text-muted-foreground">{getReleaseDecision(entry.previousRisk || 0)}</p>
                            </div>
                            <div>
                              <p className="text-xs text-muted-foreground">Current</p>
                              <p className={`font-mono font-bold ${getRiskColor(entry.riskScore)}`}>
                                {entry.riskScore}
                              </p>
                              <p className="text-xs text-muted-foreground">{entry.riskLevel}</p>
                            </div>
                          </div>
                          
                          <div className="pt-2 border-t border-muted">
                            <p className="text-sm font-medium">Decision:</p>
                            <p className={`font-medium ${getRiskColor(entry.riskScore)}`}>
                              {getReleaseDecision(entry.riskScore)}
                            </p>
                          </div>
                        </div>
                      </div>
